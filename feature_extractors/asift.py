...
class ASIFT(object):
    def __init__(self) -> None:
        ...
        for t in 2 ** (0.5 * np.arange(1, 6)):
            for phi in np.arange(0, 180, 72.0 / t):
                self.params.append((t, phi))

    def __call__(self, img):
        keypoints, descrs = [], []

        ires = [self.__asift_compute(x, img) for x in self.params]
        for i, (k, d) in enumerate(ires):
            #print(f"Affine sampling: {i + 1} / {len(params)}\r", end='')
            keypoints.extend(k)
            descrs.extend(d)
    ...
    def __asift_compute(self, p, img):
        t, phi = p
        timg, tmask, Ai = affine_skew(t, phi, img)
        keypoints, descrs = self.method.detectAndCompute(timg, tmask)
